...
export default function ProductCard({ product, viewMode = 'grid' }) {
    ...
    useEffect(() => {
        const description = document.querySelector(`#product-${_id} .product-description`);
    ...
    const handleAddToCart = async (e) => {
        ...
        try {
            ...
            const existingCartItem = cart?.items?.find(item => 
                ...
                item.variant._id === defaultVariant._id
            );

            if (existingCartItem) {
                // If item exists, increase quantity by 1
                const success = await addToCart(_id, existingCartItem.quantity + 1, defaultVariant._id);
                if (success) {
                    ...
                    <div className="product-info-list">
                        <div className="product-details-list">
                            ...
                            <div className="product-description-preview-list">
                                <MarkdownRenderer markdown={description.slice(0, 80) + (description.length > 80 ? '...' : '')} disableLinks={true} />
                            </div>
                        </div>
