...
const createProduct = async (req, res, next) => {
    try {
        const { name, description, price, variants, shipping, category, subcategory } = req.body;
        ...
        if (subcategory) {
            ...
            if (subcategoryExists.category.toString() !== category) {
                throw createError(400, "Subcategory does not belong to the selected category");
            }
        }
