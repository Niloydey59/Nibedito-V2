...
const createOrder = async (req, res, next) => {
    try {
        ...
        if (couponId) {
            ...
            if (productDiscount.type === "fixed") {
                ...
                verifiedProductDiscount =
                    (parseFloat(cart.totalPrice) * parseFloat(productDiscount.value)) / 100;
            ...
            if (shippingDiscount.type === "free") {
                ...
                finalShippingCost = parseFloat((parseFloat(shippingCost) - verifiedShippingDiscount).toFixed(2));
            ...
            verifiedShippingDiscount = parseFloat(verifiedShippingDiscount.toFixed(2));

            // Verify the discount amounts match what was sent from client
            if (
                Math.abs(verifiedProductDiscount - parseFloat(productDiscountAmount)) > 0.01 ||
                Math.abs(verifiedShippingDiscount - parseFloat(shippingDiscountAmount)) > 0.01
            ) {
                throw createError(
                    400,
        ...
        const totalDiscount = parseFloat((verifiedProductDiscount + verifiedShippingDiscount).toFixed(2));
        const calculatedFinalPrice = parseFloat((parseFloat(cart.totalPrice) - verifiedProductDiscount + finalShippingCost).toFixed(2));
        ...
        for (let item of cart.items) {
            console.log(`Processing item: Product ID ${item.product}, Variant ID ${item.variant._id}, Quantity ${item.quantity}`);
            ...
            console.log(`Found product: ${product.name}, Current totalSold: ${product.totalSold}`);
            
            let variantFound = false;
            
            for (let variant of product.variants) {
                if (variant._id.toString() === item.variant._id.toString()) {
                    console.log(`Found variant: Color ${variant.color}, Size ${variant.size}, Current stock: ${variant.quantity}`);
                    
                    if (variant.quantity < item.quantity) {
                        throw createError(400, `Not enough stock for ${product.name} (${variant.color}, ${variant.size})`);
                    }
                    
                    console.log(`Reducing stock from ${variant.quantity} to ${variant.quantity - item.quantity}`);
            ...
            console.log(`Increasing totalSold from ${product.totalSold || 0} to ${(product.totalSold || 0) + item.quantity}`);
            product.totalSold = (product.totalSold || 0) + item.quantity;
            
            await product.save();
            console.log(`Product updated successfully: ${product.name}, New totalSold: ${product.totalSold}`);
        }
...
const getAllOrders = async (req, res, next) => {
    try {
        ...
        const { status, userId, isGift, page = 1, limit = 10, sortBy = 'createdAt', order = 'desc' } = req.query;
        ...
        const processedOrders = await Promise.all(orders.map(async (order) => {
            ...
            if (orderObj.user && orderObj.user.addresses) {
                const defaultAddress = orderObj.user.addresses.find(addr => addr.isDefault);
...
const updateOrderStatus = async (req, res, next) => {
    try {
        ...
        if (status === "Cancelled" && originalOrder.status !== "Cancelled") {
            console.log(`Cancelling order ${orderId}, returning stock to inventory...`);
            // Return stock for cancelled orders
            for (const item of originalOrder.items) {
                console.log(`Processing cancelled item: Product ID ${item.product}, Variant ID ${item.variant}, Quantity ${item.quantity}`);
                const product = await Product.findById(item.product);
                if (product) {
                    console.log(`Found product: ${product.name}, Current totalSold: ${product.totalSold}`);
                    ...
                    if (variant) {
                        ...
                        console.log(`Increasing stock from ${variant.quantity} to ${variant.quantity + item.quantity}`);
                        variant.quantity += item.quantity;
                    } else {
                    ...
                    console.log(`Decreasing totalSold from ${product.totalSold || 0} to ${Math.max(0, (product.totalSold || 0) - item.quantity)}`);
...
const getUserOrders = async (req, res, next) => {
    try {
        ...
        const orders = await Order.find({ user: userId })
            .select("user items street city state phone email totalPrice isPaid createdAt dateOrdered status shippingRegion shippingCost freeShipping coupon discountAmount discountBreakdown finalPrice payment isGift giftNote") // Added isGift and giftNote
